...
\usebeamercolor{normal text}
	...
	%		}
		\begin{equation*}
			\textcolor{red}{J} =  \underbrace{\frac{1}{m} \sum_{i=1}^{m} \log D(x^{(i)}) + \log(1 - D(\textcolor{real}{G}(z^{(i)})))}_{\text{D performance}}
		\end{equation*}
	\LARGE		
	\begin{equation*}
...
\usebeamercolor{normal text}
	...
	%	\only<2>{$$ \Delta_{\theta_g} \frac{1}{m} \sum_{i=1}^{m} % \underbrace{\log(D(G(z^{(i)})))}_{\text{Generator loss}} $$}
			\begin{equation*}
			\textcolor{real}{J} = \underbrace{\frac{1}{m} \sum_{i=1}^{m} \log( \textcolor{red}{D}(G(z^{(i)})))}_{\text{G performance}}
			\end{equation*}
			\LARGE
			\begin{equation*}
			\theta_{\textcolor{real}{g}} = \theta_{\textcolor{real}{g}}  + \lambda \nabla_{\theta_{\textcolor{real}{g}}} \textcolor{real}{J}
			\end{equation*}
}
	\end{enumerate}
\end{frame} 
